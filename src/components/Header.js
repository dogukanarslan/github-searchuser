    ...
    return(
      <nav className="navbar navbar-expand navbar-dark bg-dark">
        <div className="button-container">
          ...
          <i onClick={()=>this.collapseNavbar()} className="fas fa-bars switchButton"></i>
        </div>
        <ul className={this.state.navbarClass}>
          {navLinks.map(link =>{
            return <li key={link.name} className="nav-item" onClick={()=>this.setState({navbarClass: "navbar-nav"})}><Link className="nav-link" to={link.path}>{link.name}</Link></li>
          })}
        </ul>
      </nav>
    )
